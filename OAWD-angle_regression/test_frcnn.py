...
classifier = nn.classifier(feature_map_input, roi_input, aroi_input,C.num_rois, nb_classes=len(class_mapping), trainable=True)

model_rpn = Model(img_input, rpn_layers)
model_classifier_only = Model([feature_map_input, roi_input, aroi_input], classifier)
...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
	...
	for jk in range(R.shape[0]//C.num_rois + 1):
		...
		#aROIs= np.expand_dims(R[:, 0], axis=0)
		#print(aROIs.shape())
		if ROIs.shape[1] == 0:
			break
		...
		aROIs = ROIs[:,:,:1]
		#print(aROIs.shape)
		[P_cls, P_regr] = model_classifier_only.predict([F, ROIs, aROIs])
		print("predicted......................")
		#print(P_cls)
		#print(P_regr)
		#print(P_angle)
		for ii in range(P_cls.shape[1]):
			...
			#print("enter class name",cls_name)
			...
			try:
				(tx, ty, tw, th, ta) = P_regr[0, ii, 5*cls_num:5*(cls_num+1)]
				if ta <0:
					ta = -1*ta
				else: 
					ta = ta
				probs_an.append(ta)
				probs_angle = -1*(probs_an[(np.argmax(probs_an))])
				if probs_angle <= -0.6 and probs_angle >= -1.57:
					probs_angle = -1*probs_angle
				#	if ta <0:
				#		probs_angle = ta
				#	else:
				#		probs_angle = -1*ta
				print("probs_angle", probs_angle)	
				print("ta",ta,tx,ty,tw,th)
				tx /= C.classifier_regr_std[0]
				ty /= C.classifier_regr_std[1]
				tw /= C.classifier_regr_std[2]
				th /= C.classifier_regr_std[3]
				ta /= C.classifier_regr_std[4]
				x, y, w, h = roi_helpers.apply_regr(x, y, w, h, tx, ty, tw, th)
			except:
				pass
			bboxes[cls_name].append([C.rpn_stride*x, C.rpn_stride*y, C.rpn_stride*(x+w), C.rpn_stride*(y+h), C.rpn_stride*ta])
	...
	for key in bboxes:
		...
		for jk in range(new_boxes.shape[0]):
			(x1, y1, x2, y2, a) = new_boxes[jk,:]
			#print("new boxes angle",a)
